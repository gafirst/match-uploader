  ...
  >
    ...
    <VCardText style="color: initial">
      ...
      <template v-if="!liveMode.isAllowed">
        ...
        <VList>
          <TaskListItem
            v-for="[requirement, requirementMet] in
      ...
      <VAlert
        v-if="liveMode.isAllowed && !liveMode.isActive"
        variant="outlined"
        color="success"
        density="compact"
      ...
      <template v-else>
        <h2
          v-if="liveMode.isActive"
          class="mb-2"
        >
          Live Mode is active <VChip v-if="liveMode.error || liveMode.state === LiveModeStatus.ERROR">
            Paused
          </VChip>
        </h2>
        <p
        ...
        <VAlert
          v-if="liveMode.error || liveMode.state === LiveModeStatus.ERROR"
          variant="outlined"
          color="error"
          density="compact"
        ...
        >
          <VAlertTitle class="mb-2">
            Live Mode is paused due to an error
          </VAlertTitle>
          <p class="mb-2">
            {{ liveMode.error ?? "An unknown error occurred" }}
          </p>

          <p class="mb-2">
            Once you've resolved the error, press <strong>Retry</strong> below to try again.
          </p>
          <VBtn
            class="mb-2"
            @click="liveMode.clearErrorAndTick"
          >
            Retry
          </VBtn>
        </VAlert>
        ...
        <VAlert
          v-else-if="liveMode.missingPlaylistLabels.length > 0"
          variant="outlined"
          color="warning"
          density="compact"
          icon="mdi-alert-circle"
        ...
        <VAlert
          v-if="liveMode.state === LiveModeStatus.WAITING"
          variant="outlined"
          color="purple"
          density="compact"
          icon="mdi-sleep"
        ...
        <VAlert
          v-if="liveMode.state === LiveModeStatus.ADVANCE_MATCH"
          variant="outlined"
          color="info"
          density="compact"
        ...
        <VAlert
          v-if="liveMode.state === LiveModeStatus.FETCH_VIDEOS"
          variant="outlined"
          color="info"
          density="compact"
          icon="mdi-loading mdi-spin"
        ...
        <VAlert
          v-if="liveMode.state === LiveModeStatus.QUEUE_UPLOADS"
          variant="outlined"
          color="info"
          density="compact"
          icon="mdi-loading mdi-spin"
        ...
        <VAlert
          v-if="liveMode.isActive && liveMode.state === LiveModeStatus.STOPPED"
          variant="outlined"
          color="gray"
          density="compact"
          icon="mdi-pause"
