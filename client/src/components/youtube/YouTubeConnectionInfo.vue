<template>
  <h3>
    Authentication status <VChip
      v-if="googleAuthStatus"
      :color="youtubeAuthSuccessState"
    >
      {{ googleAuthStatus }}
    </VChip>
  </h3>
  <VProgressCircular
    v-if="!youTubeAuthState"
    indeterminate
  />
  <VAlert
    v-else-if="!!error"
  ...
  <div v-else>
    <VList>
      <TaskListItem :complete="youTubeAuthState?.clientIdProvided">
        OAuth2 client ID provided
      </TaskListItem>
      <TaskListItem :complete="youTubeAuthState?.clientSecretProvided">
        OAuth2 client secret provided
      </TaskListItem>
      <TaskListItem :complete="youTubeAuthState?.accessTokenStored">
        Access token stored
      </TaskListItem>
      <TaskListItem :complete="youTubeAuthState?.refreshTokenStored">
...
const youtubeAuthSuccessState = computed(() => {
  const authStatus = props.googleAuthStatus;
  if (!authStatus) {
    return "";
  }
